...
if [ ! -f "$REPO_ROOT/dist/yt-module.tar.gz" ]; then
  "$REPO_ROOT/scripts/build.sh"
fi

scp -o ConnectTimeout=8 -o StrictHostKeyChecking=accept-new \
  "$REPO_ROOT/dist/yt-module.tar.gz" \
...
ssh -o ConnectTimeout=8 ableton@move.local '
  ...
  rm -f yt-module.tar.gz
  echo "Installed to ~/move-anything/modules/sound_generators/yt"
'
